...
class Activity implements \OCP\Activity\IExtension {
	...
	public function getNotificationTypes($languageCode) {
		...
		return array(self::TYPE_REMOTE_SHARE => $l->t('THIS WAS A REMOTE SHARE'));
	}
		...
		if ($app === 'files_sharing') {
			switch ($text) {
				case self::SUBJECT_REMOTE_SHARE_RECEIVED:
					return $l->t('You received a new server-to-server share from %1s', $params)->__toString();
				case self::SUBJECT_REMOTE_SHARE_ACCEPTED:
					return $l->t('%1$s accepted %2$s', $params)->__toString();
				case self::SUBJECT_REMOTE_SHARE_DECLINED:
					return $l->t('%1$s declined %2$s', $params)->__toString();
			}
		}
	}
